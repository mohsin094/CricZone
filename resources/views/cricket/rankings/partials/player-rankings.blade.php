            ...
            <div class="flex items-center space-x-3">
                ...
                <div>
                    <div class="flex items-center space-x-2">
                        @php
                        $topPlayerImageUrl = is_array($topPlayer) ? ($topPlayer['player_image_url'] ?? null) : ($topPlayer->player_image_url ?? null);
                        @endphp
                        @if($topPlayerImageUrl)
                        <img src="{{ $topPlayerImageUrl }}" alt="{{ is_array($topPlayer) ? ($topPlayer['player_name'] ?? 'Unknown Player') : ($topPlayer->player_name ?? 'Unknown Player') }}" class="w-8 h-8 player-avatar rounded-full object-cover">
                        @endif
                        <div>
                            ...
                            <div class="flex items-center space-x-1">
                                @php
                                $topTeamFlagUrl = is_array($topPlayer) ? ($topPlayer['team_flag_url'] ?? null) : ($topPlayer->team_flag_url ?? null);
                                @endphp
                                @if($topTeamFlagUrl)
                                <img src="{{ $topTeamFlagUrl }}" alt="{{ is_array($topPlayer) ? ($topPlayer['team_name'] ?? 'Unknown Team') : ($topPlayer->team_name ?? 'Unknown Team') }}" class="w-4 h-3 team-flag">
                                @endif
                ...
                <tr class="hover:bg-gray-50">
                    ...
                    <td class="px-2 py-1">
                        <div class="flex items-center">
                            @php
                            $playerImageUrl = is_array($ranking) ? ($ranking['player_image_url'] ?? null) : ($ranking->player_image_url ?? null);
                            @endphp
                            @if($playerImageUrl)
                            <img src="{{ $playerImageUrl }}" alt="{{ is_array($ranking) ? ($ranking['player_name'] ?? 'Unknown Player') : ($ranking->player_name ?? 'Unknown Player') }}" class="w-5 h-5 player-avatar rounded-full object-cover mr-1">
                            @endif
